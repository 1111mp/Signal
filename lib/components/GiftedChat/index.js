...
import {utils, MessageContainer} from 'react-native-gifted-chat';
...
import {ACCESSORY_HEIGHT} from './InputToolbar';
...
export default class GiftedChat extends React.Component {
  constructor(props) {
    super(props);
    this._accessoryShow = false;
    this._composerHeight = this.props.minComposerHeight;
    ...
    this.state = {
      ...
      accessoryOpacity: new Animated.Value(0),
      ...
      if (this.props.isKeyboardInternallyHandled) {
        ...
        if (this.getAccessoryShow()) {
          ...
          Animated.parallel([
            ...
            Animated.timing(this.state.accessoryOpacity, {
              toValue: 0,
              duration: e.duration,
              useNativeDriver: false,
            }),
          ]).start(() => {
            this.setAccessoryShow(false);
          });
        } else {
          this.state.messagesContainerHeight.setValue(
    ...
    this.onKeyboardWillHide = _e => {
      if (this.props.isKeyboardInternallyHandled) {
        ...
        if (this.getAccessoryShow()) {
          Animated.parallel([
            Animated.timing(this.state.messagesContainerHeight, {
              toValue: newMessagesContainerHeight - ACCESSORY_HEIGHT,
              duration: 100,
              useNativeDriver: false,
              easing: Easing.out(Easing.quad),
            }),
    ...
    this.onInputSizeChanged = size => {
      ...
      // remember value of the composerHeight
      this._composerHeight = newComposerHeight;
      this.setState({
        composerHeight: newComposerHeight,
        // messagesContainerHeight: newMessagesContainerHeight,
      });
      this.state.messagesContainerHeight.setValue(newMessagesContainerHeight);
    };
  ...
  setAccessoryShow(value) {
    this._accessoryShow = value;
  }
  getAccessoryShow() {
    return this._accessoryShow;
  }
  ...
  closeAccessoryHandler = () => {
    if (this.getAccessoryShow()) {
      Animated.parallel([
        Animated.timing(this.state.messagesContainerHeight, {
          toValue: this.getBasicMessagesContainerHeight(),
          duration: 250,
          useNativeDriver: false,
          easing: Easing.out(Easing.quad),
        }),
  ...
  pressMoreHandler = ({isSound = false, soundShow = false}) => {
    if (this.getAccessoryShow()) {
      // opened to close
      if (this._keyboardHeight <= 0 && !isSound) {
        this.focusTextInput();
      } else {
        const newComposerHeight = isSound
          ? this.props.minComposerHeight
            ...
            Animated.timing(this.state.messagesContainerHeight, {
              toValue:
                this._keyboardHeight > 0
                  ...
                  : this.getBasicMessagesContainerHeight(newComposerHeight),
              duration: 250,
              useNativeDriver: false,
              easing: Easing.out(Easing.circle),
            }),
      ...
      // closed to open
      if (this._keyboardHeight > 0) {
        this.setAccessoryShow(true);
        Keyboard.dismiss();
      } else {
        Animated.parallel([
          Animated.timing(this.state.messagesContainerHeight, {
            toValue:
              this.getBasicMessagesContainerHeight(this._composerHeight) -
              ACCESSORY_HEIGHT,
            duration: 250,
            useNativeDriver: false,
            easing: Easing.out(Easing.quad),
  ...
  setInputToolBarToInitial = (initial = false) => {
    const newComposerHeight = initial
      ? this.props.minComposerHeight
      : this._composerHeight;
    const newMessagesContainerHeight = this.getMessagesContainerHeightWithKeyboard(
      newComposerHeight,
    );
    this.setState(
      ...
      () => {
        if (this._keyboardHeight <= 0 && !initial) {
          this.focusTextInput();
        }
        if (this._keyboardHeight > 0 && initial) {
          Keyboard.dismiss();
  ...
  renderInputToolbar() {
    const inputToolbarProps = {
      ...
      onSetInputToolBarToInitial: this.setInputToolBarToInitial,
    ...
    return (
      <InputToolbar
        {...inputToolbarProps}
        pressMoreHandler={this.pressMoreHandler}
        accessoryOpacity={this.state.accessoryOpacity}
      />
    );
  }
